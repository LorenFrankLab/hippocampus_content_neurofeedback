...
class RealtimePosteriorSumParameters(dj.Manual):
    ...
    def insert_default(self):
        ...
        self.insert1(
            ["default", posterior_sum_parameters], skip_duplicates=True
        )
...
class RealtimePosteriorSumSelection(dj.Manual):
    definition = """
    -> RealtimeFilename
    -> RealtimePosteriorSumParameters
    ---
    """
...
class RealtimePosteriorSum(dj.Computed):
    ...
    def make(self, key):
        ...
        # flag for no_reward_time version
        no_reward_time = 0
        try:
            no_reward_time = posterior_sum_parameters["no_reward_time"]
        ...
        # reward count
        reward_counts = stim_message[
            stim_message["shortcut_message_sent"] == 1
        ].shape[0]
        print(
            "rewards",
        ...
        # if excluding reward 30sec need to do calcualtions here
        if no_reward_time == 1:
            print("no reward version")
            ...
            stim_message_diff_reward[
                "timestamp_diff"
            ] = stim_message_diff_reward["bin_timestamp"].diff()
            ...
            print(stim_message_diff_reward_2.shape)
            task2_head_dir_reward = pd.merge_asof(
                task2_head_dir,
                stim_message_diff_reward_2,
                on="bin_timestamp",
                direction="nearest",
                tolerance=50,
            )

            task2_reward_shortcut_sent = task2_head_dir_reward[
                task2_head_dir_reward["shortcut_message_sent"] == 1
            ]
            print(task2_reward_shortcut_sent.shape)

            good_timestamp = task2_reward_shortcut_sent["bin_timestamp"].values

            reward_timestamps_30ms = []
            for i in np.arange(5):
                timestamps = good_timestamp - 180 * i
                reward_timestamps_30ms.append(timestamps.tolist())

            reward_timestamps_30ms_combined = np.array(
                [val for sublist in reward_timestamps_30ms for val in sublist]
            )

            counter2 = 0
            matching_timestamps = []
            for counter in np.arange(task2_head_dir.shape[0]):
                if (
                    task2_head_dir["bin_timestamp"][counter]
                    in reward_timestamps_30ms_combined
                ):
                    # print('matching_timestamp')
                    counter2 += 1
                    matching_timestamps.append([1])
                else:
                    matching_timestamps.append([0])
            print(counter2)

            task2_head_dir["reward_30ms"] = np.array(matching_timestamps)

            task2_no_pre_reward = task2_head_dir[
                (task2_head_dir["reward_30ms"] == 0)
            ]
            print("all task2 center, no reward 30ms", task2_no_pre_reward.shape)

            task2_dataframe = task2_no_pre_reward.copy()
        else:
            print("all task2 time")
            task2_dataframe = task2_decode_center.copy()

        # number of bins with >0.4 of posterior in arm end
        if task2_dataframe.shape[0] > 0:
            print(
                ">0.4 arm1 end",
                task2_dataframe[task2_dataframe["arm1_end"] > 0.4].shape[0]
                / task2_dataframe.shape[0],
            )
            print(
                ">0.4 arm2 end",
                task2_dataframe[task2_dataframe["arm2_end"] > 0.4].shape[0]
                / task2_dataframe.shape[0],
            )
            arm1_end = (
                task2_dataframe[task2_dataframe["arm1_end"] > 0.4].shape[0]
            ...
            print(
                ">0.4 arm1 base",
                task2_decode_center[
                    task2_decode_center["arm1_base"] > 0.4
                ].shape[0]
                / task2_decode_center.shape[0],
            )
            print(
                ">0.4 arm2 base",
                task2_dataframe[task2_dataframe["arm2_base"] > 0.4].shape[0]
                / task2_dataframe.shape[0],
            )
            arm1_base = (
                task2_dataframe[task2_dataframe["arm1_base"] > 0.4].shape[0]
                / task2_dataframe.shape[0]
            )
            arm2_base = (
                task2_dataframe[task2_dataframe["arm2_base"] > 0.4].shape[0]
            ...
            arm1_total = (
                task2_dataframe["arm1_all"].sum() / task2_dataframe.shape[0]
            )
            arm2_total = (
                task2_dataframe["arm2_all"].sum() / task2_dataframe.shape[0]
            )
            ...
            if no_reward_time == 1:
                task2_center_velocity = np.mean(task2_dataframe["velocity_x"])
            else:
                task2_center_velocity = np.mean(task2_dataframe["velocity"])
        ...
        print(
            ...
            stim_message_no_reward[
                ...
                & (
                    stim_message_no_reward["center_well_dist"]
                    < well_distance_max
                )
        ...
        if task2_decoder.shape[0] > 0:
            ...
            # for normal
            reward_rate = (
                stim_message[stim_message["shortcut_message_sent"] == 1].shape[
                    0
                ]
                / task2_time_all
            )
